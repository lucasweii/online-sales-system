...
public:
    METHOD_LIST_BEGIN
        ...
        ADD_METHOD_TO(Customer::searchByName, "/search", Post, "Log", "SessionControl");
    METHOD_LIST_END
    ...
    void searchByName(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback);
private:
    struct Order {
        std::string goods_id;
        std::string num;
